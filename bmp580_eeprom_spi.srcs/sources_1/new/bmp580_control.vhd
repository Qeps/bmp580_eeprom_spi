...
architecture rtl of bmp580_control is
    ...
    type state_t is (
        ...
        --      b) OSR set - validation
        READ_SEND_OSR_CONFIG_ADDR,
        READ_SEND_OSR_CONFIG_DUMMY,
        READ_WAIT_OSR_CONFIG_DATA_RX,
        --      c) ODR - (deep_dis=0, odr=1Hz, pwr_mode=01)
    ...
    signal init_delay_cnt      : unsigned(18 downto 0)         := (others => '0');
    signal standby_delay_cnt   : unsigned(18 downto 0)         := (others => '0');
    signal normal_delay_cnt    : unsigned(18 downto 0)         := (others => '0');
    signal spi_init_flag       : std_logic                     := '0';
    signal spi_debug_reg       : std_logic                     := '0';

begin

    spi_debug       <= spi_debug_reg;
                ...
                case state is
                    ...
                    when WAIT_RX =>
                        if rx_data_valid = '1' then
                            case rx_wait_ctx is
                                ...
                                when READ_WAIT_OSR_CONFIG_ADDR_RX =>
                                    state <= READ_SEND_OSR_CONFIG_DUMMY;
                                when OTHERS =>
                                    state <= IDLE;
                            end case;
                        end if;
                    ...
                    when WAIT_T_NORMAL_MS =>
                        if normal_delay_cnt < to_unsigned(NORMAL_DELAY_CYCLES - 1, normal_delay_cnt'length) then
                            ...
                            normal_delay_cnt <= (others => '0');
                            state            <= READ_SEND_OSR_CONFIG_ADDR;
                        end if;
                    -- 5.b
                    when READ_SEND_OSR_CONFIG_ADDR =>
                        if tx_ready = '1' then
                            tx_count_reg      <= x"02";
                            tx_byte_reg       <= OSR_CONFIG_ADDR or READ_DATA;
                            tx_data_valid_reg <= '1';
                            rx_wait_ctx       <= READ_WAIT_OSR_CONFIG_ADDR_RX;
                            state             <= WAIT_RX;
                        end if;
                    when READ_SEND_OSR_CONFIG_DUMMY =>
                        if tx_ready = '1' then
                            tx_byte_reg       <= DUMMY_BYTE;
                            tx_data_valid_reg <= '1';
                            state             <= READ_WAIT_OSR_CONFIG_DATA_RX;
                        end if;
                    when READ_WAIT_OSR_CONFIG_DATA_RX =>
                        if rx_data_valid = '1' then
                            if rx_byte = CFG_OSR_TEMP_ONLY then
                                spi_debug_reg        <= '1';
                            else
                                spi_debug_reg        <= '0';
                            end if;
